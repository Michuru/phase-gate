...
9. **Display what's still open** — after wrapping up, show the remaining open items across *all* sections of `BACKLOG.md`, not just the one just worked, so the user can pick the next one without asking. Do this every time, automatically. **Presentation**: a short numbered list, one line per item, with the just-resolved item still shown in its place but struck through (`~~like this~~`) rather than silently dropped — so the user sees the before/after in one glance. Keep it to a single short list; don't re-expand any item's full investigation detail here.
