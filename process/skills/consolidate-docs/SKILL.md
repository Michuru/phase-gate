...
1. Read `CLAUDE.md` in full and identify, separately, (a) sections that have grown past "what is this and where do I go for more" into full narrative history, and (b) individual rules that may be stale (see "Staleness" above). Both are judgment calls, not transcription — do them in the main session, not `docs-writer`. **Before this manual pass, consider whether a staleness-detection script would give a useful pre-filtered candidate list** — computing each rule's introduction date via `git log -S'<distinctive lead phrase>'` (survives a reflow, unlike a line-blame date, since it tracks the phrase's own first appearance) and/or a citation date already inline in the rule's own text; build one for your own repo if none exists yet, rather than writing the same date-tracking tag into `CLAUDE.md` itself (which a future edit to that rule would just reset). A triage signal only, never a verdict — every flagged rule still goes through this step's own read-and-verify judgment.
...
1. Read `MISTAKES.md` in full and identify candidates against the three shapes above — for the age-out shape, check each entry's own date against today directly (don't estimate) and confirm its shape genuinely never recurred. Verify each one directly (read the named `CLAUDE.md` section; Grep/Glob for the claimed-gone tool) before treating it as confirmed. **Also ask, for any candidate whose lesson still lives only as prose, whether the mistake's shape could instead be caught by a small script + hook** — build that first if a mechanization is still missing, rather than letting an unmechanized lesson disappear into archived history. Judgment call — stays in the main session, not `docs-writer`.
