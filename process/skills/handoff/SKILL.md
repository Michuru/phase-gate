...
**The closed-thread check itself** (run once per candidate, whether there's one or several): read the primer, then **cheaply check whether its own thread is already closed** before presenting it as authoritative. Two things to check, both targeted reads/greps of what the primer already points at — never a fresh investigation. **Run check 1 first; only run check 2 if check 1 came back inconclusive** (no marker either way) — an explicit marker already settles it, so there's no need to also grep git log once one's found:
...
2. **If check 1 was inconclusive, also check the primer's own named repo for the work having shipped without the primer ever being resumed** — a targeted `git log --oneline` (optionally `--since=<primer's own date>`) on the repo the primer's named file(s) live in, scanning for a commit message that plausibly completes the exact feature/task the primer describes. Absence of a Status field or a `BACKLOG_ARCHIVE.md` entry is **not** proof the thread is still open — it may just mean nobody wrote a completion marker even though the work shipped by a different path than resuming this primer.
