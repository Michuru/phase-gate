...
- **Tier 1** — preference order (1) a genuinely different-family model (e.g. Fable), when available; (2) a different Claude model than the drafting one (sonnet- or haiku-drafted → `model: opus`; opus-drafted → `model: sonnet`); (3) the same model in a fresh context, only when nothing else is available at all.
- **Tier 2** — preference order (1) a different Claude model than the drafting one (sonnet- or haiku-drafted → `model: opus`; opus-drafted → `model: sonnet`, the most common pairing); (2) the same model in a fresh context, only when no other Claude model is available. A genuinely different-family model stays reachable for a Tier 2 design, just not automatically — it's requestable as an explicit further pass via Step 3.5's option (c), the same way any "one more look" already works.

Weakest option in either ladder is still real, never labeled as more than it is. Every Claude-named option is a Claude model; what buys the independence is the fresh context plus a different model than the author, not a different vendor. Check your own recorded model-availability configuration before picking a target — resolved upward from the current repo if this session is in a nested sub-repo (same rule `initiate/SKILL.md` Step 1.2 uses) — never assume a different-family model is available for Tier 1, and never reach for one automatically on Tier 2. **The heading names the actual reviewing model every time** (`## Design review — Fable`, `## Design review — Opus`, `## Design review — Sonnet, fresh context`) and claims nothing else; a same-model pass is never labeled cross-model. The review still runs even when only the weakest option is reachable — silently skipping for lack of a stronger independence reverses the whole point of this step.
